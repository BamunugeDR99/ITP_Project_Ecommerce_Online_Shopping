...
export default function AllItemsFiltered(props) {
  ...
  useEffect(() => {
    async function getItems() {
      ...
      axios
        ...
        .then((res) => {
          if (objectId == "Apple" || objectId == "Samsung" || objectId == "Google" || objectId == "Google" || objectId == "Sony") {
            
            filter_item = res.data.filter(
              (item) => item.Category === objectId || item.Brand === objectId
            );

          } else if (objectId == "Mobile Phones" || objectId == "Gaming" || objectId == "Laptops") {
            
            filter_item = res.data.filter(
              (item) => item.Category === objectId || item.Category === objectId
              );

          }else if(objectId == "Tablet"){
            
           let  objectIds = "Tablet / iPad / iPod";
            filter_item = res.data.filter(
                (item) => item.Category === objectIds || item.Category === objectIds
                );

          }

          console.log(filter_item);
          res.data = filter_item;
          setItems(res.data);
