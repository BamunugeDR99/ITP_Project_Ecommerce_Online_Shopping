...
export default function PackageMonthlySales(props) {
 ...
 // const [totalStock, setTotalStock] = useState("");
  ...
  function CreateFinalResult(content) {
    for (let i = 0; i < content.length; i++) {
      ...
      let totalPrice = content[i].Packages[0].price * content[i].OQuantity;
      let order = {
        OrderDate: fullMonth,
        OrderID: content[i].OrderID,
        ItemName: content[i].Packages[0].packageName,
        Description: content[i].Packages[0].description,
        Quantity: content[i].OQuantity,
        UnitPrice: content[i].Packages[0].price + ".00",
        TotalPrice: totalPrice + ".00",
      };

      //totalStocks = totalStocks + parseFloat(content[i].OQuantity);
      incomes += parseFloat(totalPrice);
      outgoings += parseFloat(content[i].Packages[0].price * 0.1);
    ...
    for (let i = 0; i < content.length; i++) {
     ...
     // result.push(orderAmounts);
      ...
      .then((res) => {
        ...
        saveAs(pdfBlob, "MonthlyPackageSalesReport.pdf");
                          //your file name 
      });
  ...
  return (
    <div>
      <br></br>
      <h2 style={{ textAlign: "center" }}>Monthly Package Sales</h2>
      <br></br>
      Currency Code : LKR
      <br></br>
      <h3 style={{ float: "right" , marginRight:"7 rem" }}> TODAY : {dates}</h3>
      <br></br>
      <br></br>
          ...
          <div class="card-body">
            <div class="container">
              <div class="row">
                ...
                <div class="col-sm">Total Packages : {totalItems}</div>
