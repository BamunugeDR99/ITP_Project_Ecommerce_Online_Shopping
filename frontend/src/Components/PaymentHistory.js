...
export default function PaymentHistory(props) {
  ...
  function filterpurchaseHistory(data, purchaseHistorySearch) {
    let result = data.filter(
      (post) =>
        post.RecieptNo.toLowerCase().includes(
          purchaseHistorySearch.toLowerCase()
        ) ||
        post.PaymentType.toLowerCase().includes(
          purchaseHistorySearch.toLowerCase()
        )
    );
    ...
    <div className="container">
      ...
      <div class="input-group" id="SellSerch" style={{ width: "1200px" }}>
        <input
          ...
          placeholder="Search by Recipt Number or Payment Type "
          aria-label="Search"
          ...
          return (
            ...
            <tbody>
              <tr>
                ...
                <td>
                  {/* <button  class="read-more-btn btn-primary"style = {{marginRight : "10px"}} onClick={()=>gotoShowMore(payhistory._id)}>Show More</button> */}
                </td>
              </tr>
            </tbody>
            // </div>
